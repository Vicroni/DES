...
public class Sustituidor {
     ...
     */
    public String sustitucion(String cadena) {
     ...
     */
    private String sustitucion_x_caja(String bits_6, int ref) {
        int[][] s_box = this.S[ref];
        StringBuilder filaBuilder = new StringBuilder();
        filaBuilder.append(bits_6.charAt(0));
        filaBuilder.append(bits_6.charAt(5));

        int fila = new BigInteger(filaBuilder.toString(), 2).intValue();
        int columna = new BigInteger(bits_6.substring(1, 5), 2).intValue();
        int s = s_box[fila][columna];
        String s_binario = Integer.toString(s, 2);
        if (s_binario.length() != 4) {
            int n = 4 - s_binario.length();
            for (int i = 0; i < n; i++) {
